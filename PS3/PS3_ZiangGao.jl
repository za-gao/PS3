...
# ╔═╡ 3a6e522b-2a2b-4723-a9cb-1cdac6e28a35
md"#### ChemE7770, Problem Set 3, Ziang Gao, zg276"
...
# ╔═╡ 492e8d71-0cb8-45cb-8549-4c701bf6ae3a
md"###### The mean and median value of [Met]/Km for all the Homo sapiens data in the Park dataset were calculated, I then decide to use the median value of the dataset to represent all unknown values and compute the fluxes corresponding to each reaction, which is 4.85"

# ╔═╡ 30263759-e3c3-4c22-a270-0e7f8d70e35c
md"###### Using the provided formula to calculate flux array"

# ╔═╡ abefdd55-6a81-4058-b49b-5e4991d085af
md"###### This didn't affect the result from C1, because the rate limiting step v2 is not bounded in the case. However, if use the equation without -1 in the denominator instead, with all other setups unchanged, the resulting flux analysis is different:"
...
# ╔═╡ 696655c7-d114-448d-afe8-3d6089d42566
begin

Organism_Target = ["Homo sapiens"]
filter_col_key2 = Symbol("Organism")
df2 = filter(filter_col_key2=>x->in(x,Organism_Target), metabolite_table)
A1 = mean(df2."[Met]/Km")
A2 = median(df2."[Met]/Km")
	
end

# ╔═╡ f69836b0-26d0-4ba9-aa93-2a2e339c2759
begin
	# now calculate the value in the bracelet using the provided equation
	
	B = ones(6)
	B[1] = 11.9*96.7*4.85/((1+11.9)*(1+97.7)*(1+4.85)-1)
	B[2] = 4.85/((1+4.85)-1)
	B[3] = 0.165*4.85/((0.165+1)*(4.85+1)-1)
	# here I use the data for Saccharomyces cerevisiae organism to calculate the data
	B[4] = 2.81*4.85/((2.81+1)*(4.85+1)-1)
	
	# stoichiometries are taken consider for reaction v5, using Eqn(3) in the Klipp's paper
	
	B[5] = 73.1^2*4.85^4*4.85^3*4.85^3/((1+73.1+73.1^2)*(1+4.85+4.85^2+4.85^3+4.85^4)*(1+4.85+4.85^2+4.85^3)*(1+4.85+4.85^2+4.85^3)-1)
	B[6] = 4.85^2*4.85^2*4.85^3*4.85^4/((1+4.85+4.85^2)*(1+4.85+4.85^2)*(1+4.85+4.85^2+4.85^3)*(1+4.85+4.85^2+4.85^3+4.85^4)-1)

	B
end

# ╔═╡ 521bd228-0f14-47f8-b8f0-4ff15d6ef75e
begin
	
	# setup flux bounds array for this question -
	Flux_bounds_array = zeros(ℛ,2)
	Flux_bounds_array[:,2] .= 100.0 # default value is 100 for flux units: μmol/gDW-s
	Flux_bounds_array[1,2] = 203.0*E[1]*B[1]
	Flux_bounds_array[2,2] = 34.5*E[2]*B[2]
	Flux_bounds_array[3,2] = 249.0*E[3]*B[3]
	Flux_bounds_array[4,2] = 88.1*E[4]*B[4]
	Flux_bounds_array[5,2] = 13.7*E[5]*B[5]
	Flux_bounds_array[6,2] = 13.7*E[6]*B[6]
...
# ╔═╡ 7d2d0c8d-e451-483e-8fdb-02e6ee81367f
begin
	# compute the flux -
	result_case_2 = lib.flux(S,Flux_bounds_array,species_bounds_array,c_vector);
...
# ╔═╡ 57ed2253-3a02-4c8f-9ac7-306a62413d24
let
...
# ╔═╡ 90a9cebc-35b4-4cc2-853a-134db5ca5216
begin
	# now calculate the value in the bracelet using a modified equation
	
	M_B = ones(6)
	M_B[1] = 11.9*96.7*4.85/((1+11.9)*(1+97.7)*(1+4.85))
	M_B[2] = 4.85/((1+4.85))
	M_B[3] = 0.165*4.85/((0.165+1)*(4.85+1))
	# here I use the data for Saccharomyces cerevisiae organism to calculate the data
	M_B[4] = 2.81*4.85/((2.81+1)*(4.85+1))
	# stoichiometries are taken consider for reaction v5, using Eqn(3) in the Klipp's paper
	M_B[5] = 73.1^2*4.85^4*4.85^3*4.85^3/((1+73.1+73.1^2)*(1+4.85+4.85^2+4.85^3+4.85^4)*(1+4.85+4.85^2+4.85^3)*(1+4.85+4.85^2+4.85^3))
	M_B[6] = 4.85^2*4.85^2*4.85^3*4.85^4/((1+4.85+4.85^2)*(1+4.85+4.85^2)*(1+4.85+4.85^2+4.85^3)*(1+4.85+4.85^2+4.85^3+4.85^4))

	#M_B
	# setup flux bounds array for this question -
	M_Flux_bounds_array = zeros(ℛ,2)
	M_Flux_bounds_array[:,2] .= 100.0 # default value is 100 for flux units: μmol/gDW-s
	M_Flux_bounds_array[1,2] = 203.0*E[1]*M_B[1]
	M_Flux_bounds_array[2,2] = 34.5*E[2]*M_B[2]
	M_Flux_bounds_array[3,2] = 249.0*E[3]*M_B[3]
	M_Flux_bounds_array[4,2] = 88.1*E[4]*M_B[4]
	M_Flux_bounds_array[5,2] = 13.7*E[5]*M_B[5]
	M_Flux_bounds_array[6,2] = 13.7*E[6]*M_B[6]

	# O2 uptake -
	M_Flux_bounds_array[15,1] = 0.25

	M_B[6]
	#M_Flux_bounds_array
end

# ╔═╡ 75b96772-bd6e-4a4f-aa75-9ecb4cb5f3f1
begin
	# compute the flux -
	result_case_M2 = lib.flux(S,M_Flux_bounds_array,species_bounds_array,c_vector);

	# check:
	with_terminal() do
	ef = result_case_M2.exit_flag
	sf = result_case_M2.status_flag
...
# ╔═╡ eeb3da87-242f-4c6e-bf31-959942fe4003
let

	# get flux values from the result -
	MCalculated_flux_array = result_case_M2.calculated_flux_array

	# build flux table -
	MFlux_table = Array{Any,2}(undef,ℛ,4)

	# populate -
	for i ∈ 1:ℛ
		MFlux_table[i,1] = i
		MFlux_table[i,2] = rna[i]
		MFlux_table[i,3] = MCalculated_flux_array[i]*(1) # units: μmol/gDW-hr
		MFlux_table[i,4] = expanded_reaction_array[i]
	end
	...
	with_terminal() do
		pretty_table(MFlux_table; header=Header_row, alignment=:l)
	end
end
